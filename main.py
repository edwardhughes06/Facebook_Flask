...
class PostLog(db.Model):
	...
	subject_text = db.Column(db.String(80))
	post_text = db.Column(db.String(300))
...
def appendIfNotIn(list, entry, list2=None,):
...
def find_new_friends(current_user_id):
...
def return_search_results_friends(search_entry):
	friend_search = User.query.filter(
		or_(func.lower(User.username).contains(func.lower(search_entry)),
			func.lower(User.first_name).contains(func.lower(search_entry)))).all()
	results_list = []
	for results in friend_search:
		if int(results.id) != int(current_user.get_id()):
	...
	if current_user.is_authenticated:
		...
		if user:
			if check_password_hash(user.password, form.password.data):
				...
				return redirect(url_for('dashboard',refresh_data=True))

		return '<h1>Invalid Username or password</h1>'

	return render_template('login.html', form=form)

@app.route('/cardTest/', methods=['GET', 'POST'])
def cardTest():
...
def home():
	return render_template('home.html', first_name=current_user.username)

@app.route('/dashboard/<refresh_data>', methods=['GET', 'POST'])
@app.route('/dashboard/', methods=['GET', 'POST'])
@login_required
def dashboard(refresh_data = None):
	search_results = None
	search_ids = []
